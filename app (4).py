...
# ─────────────────────────────────────────────
#  HTML HELPERS  (unchanged from your original)
# ─────────────────────────────────────────────
def base_style():
    return """
<style>
...
# ─────────────────────────────────────────────
#  PAGE BUILDERS  (100% preserved from your file)
# ─────────────────────────────────────────────
def page_landing():
    body = """
<nav>
    ...
    <div class="form-card no-print" style="{form_style}">
      ...
      <form method="POST" action="/doctor/patient/{p['id']}/examine">
        <div style="margin-bottom:1.1rem">
          <label style="display:block;font-size:.75rem;color:rgba(255,255,255,.45);margin-bottom:.45rem;letter-spacing:.06em;text-transform:uppercase">Diagnosis *</label>
        ...
        <div style="margin-bottom:1.1rem">
          <label style="display:block;font-size:.75rem;color:rgba(255,255,255,.45);margin-bottom:.45rem;letter-spacing:.06em;text-transform:uppercase">Prescription *</label>
        ...
        <div style="margin-bottom:1.5rem">
          <label style="display:block;font-size:.75rem;color:rgba(255,255,255,.45);margin-bottom:.45rem;letter-spacing:.06em;text-transform:uppercase">Doctor Notes <span style="color:rgba(255,255,255,.2);font-size:.7rem">(optional)</span></label>
...
def page_statistics(doctor_name, patients):
    ...
    conf_m = json.dumps([_col.Counter(
        datetime.strptime(p["submitted_at"][:10],"%Y-%m-%d").strftime("%b")
        for p in patients if p["status"]=="Confirmed"
    ).get(m,0) for m in months_order])
    canc_m = json.dumps([_col.Counter(
        datetime.strptime(p["submitted_at"][:10],"%Y-%m-%d").strftime("%b")
        for p in patients if p["status"]=="Cancelled"
    ).get(m,0) for m in months_order])
...
.kpi-lbl{{font-size:.7rem;color:rgba(255,255,255,.55);text-transform:uppercase;letter-spacing:.07em;margin-bottom:.3rem}}
.kpi-val{{font-size:2.4rem;font-weight:800;line-height:1.1;margin:.15rem 0 .3rem;display:block}}
.kpi-note{{font-size:.75rem;color:rgba(255,255,255,.4)}}
.charts-grid{{display:grid;grid-template-columns:1fr 1fr;gap:1.1rem}}
.chart-card{{background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.08);border-radius:16px;padding:1.1rem;min-width:0}}
.chart-title{{font-size:.85rem;font-weight:700;color:rgba(255,255,255,.8);margin-bottom:.85rem;display:flex;align-items:center;gap:.5rem;flex-wrap:wrap}}
.chart-sub{{font-size:.7rem;color:rgba(255,255,255,.35);margin-left:auto}}
.jnav{{display:inline-flex;align-items:center;gap:.3rem;padding:.3rem .75rem;border-radius:100px;font-size:.78rem;font-weight:500;color:rgba(255,255,255,.5);text-decoration:none;border:1px solid rgba(255,255,255,.1);background:rgba(255,255,255,.04);transition:all .2s;white-space:nowrap}}
.jnav:hover{{background:rgba(13,148,136,.2);border-color:rgba(13,148,136,.5);color:#14b8a6}}
.sec-hdr{{font-size:.78rem;text-transform:uppercase;letter-spacing:.1em;color:rgba(255,255,255,.4);margin-bottom:.6rem;display:flex;align-items:center;gap:.5rem;font-weight:600}}
.sec-bar{{display:inline-block;width:3px;height:14px;border-radius:2px;flex-shrink:0}}
@media(max-width:1100px){{.kpi-grid{{grid-template-columns:repeat(3,1fr)}}}}
@media(max-width:800px){{.kpi-grid{{grid-template-columns:repeat(2,1fr)}}.charts-grid{{grid-template-columns:1fr}}}}
</style>

<div class="sidebar">
  <div class="s-head">
    <div style="display:flex;align-items:center;gap:.6rem;margin-bottom:.5rem">
      <div style="width:34px;height:34px;background:linear-gradient(135deg,var(--teal),var(--teal-light));border-radius:9px;display:flex;align-items:center;justify-content:center;font-size:1rem">🏥</div>
  ...
  <div class="s-foot">
    <a href="/doctor/logout" style="display:flex;align-items:center;gap:.5rem;color:rgba(255,255,255,.3);font-size:.83rem;text-decoration:none;padding:.4rem;border-radius:8px;transition:all .2s"
...
<div class="smain">
  ...
  <div style="position:sticky;top:53px;z-index:40;background:rgba(10,22,40,.97);backdrop-filter:blur(10px);border-bottom:1px solid rgba(255,255,255,.08);padding:.55rem 1.5rem;display:flex;align-items:center;gap:.4rem;flex-wrap:wrap">
    ...
    <a href="#sec-monthly"   class="jnav">Monthly</a>
    ...
    <div id="sec-overview">
      <div class="kpi-grid">
        <div class="kpi" style="background:rgba(13,148,136,.12);border-color:rgba(13,148,136,.4)">
          <div class="kpi-lbl">Total Patients</div><div class="kpi-val" style="color:#14b8a6">{total}</div><div class="kpi-note">All records</div>
        </div>
        <div class="kpi" style="background:rgba(34,197,94,.10);border-color:rgba(34,197,94,.4)">
          <div class="kpi-lbl">Examined</div><div class="kpi-val" style="color:#4ade80">{examined}</div><div class="kpi-note">Rate: {exam_rate}%</div>
        </div>
        <div class="kpi" style="background:rgba(212,168,83,.10);border-color:rgba(212,168,83,.4)">
          <div class="kpi-lbl">Pending</div><div class="kpi-val" style="color:#fbbf24">{pending}</div><div class="kpi-note">Awaiting</div>
        </div>
        <div class="kpi" style="background:rgba(59,130,246,.10);border-color:rgba(59,130,246,.4)">
          <div class="kpi-lbl">Confirmed</div><div class="kpi-val" style="color:#60a5fa">{confirmed}</div><div class="kpi-note">Ready</div>
        </div>
        <div class="kpi" style="background:rgba(239,68,68,.10);border-color:rgba(239,68,68,.4)">
          <div class="kpi-lbl">Cancelled</div><div class="kpi-val" style="color:#f87171">{cancelled}</div><div class="kpi-note">Did not proceed</div>
        </div>
      </div>
    </div>

    <div id="sec-monthly">
      <div class="sec-hdr"><span class="sec-bar" style="background:#14b8a6"></span>Monthly Admissions</div>
      <div class="chart-card"><div class="chart-title">Admissions per Month</div><canvas id="chartMonthly" height="120"></canvas></div>
    </div>

    <div id="sec-status">
      <div class="sec-hdr"><span class="sec-bar" style="background:#4ade80"></span>Examined vs Pending</div>
      <div class="chart-card"><div class="chart-title">Examined vs Pending by Month</div><canvas id="chartStatus" height="120"></canvas></div>
    </div>

    <div class="charts-grid">
      <div id="sec-symptoms">
        <div class="sec-hdr"><span class="sec-bar" style="background:#f472b6"></span>Top Symptoms</div>
        <div class="chart-card"><div class="chart-title">Symptom Frequency</div><canvas id="chartSymptoms" height="160"></canvas></div>
      </div>
      <div id="sec-diagnoses">
        <div class="sec-hdr"><span class="sec-bar" style="background:#818cf8"></span>Top Diagnoses</div>
        <div class="chart-card"><div class="chart-title">Diagnosis Frequency</div><canvas id="chartDiagnoses" height="160"></canvas></div>
      </div>
    </div>

    <div id="sec-dow">
      <div class="sec-hdr"><span class="sec-bar" style="background:#f97316"></span>Appointments by Day of Week</div>
      <div class="chart-card"><div class="chart-title">Busiest Days</div><canvas id="chartDow" height="110"></canvas></div>
    </div>
  </div>
</div>
...
function opts(){{return{{responsive:true,plugins:{{legend:{{display:false}},tooltip:{{backgroundColor:'rgba(10,22,40,.95)',titleColor:'#fff',bodyColor:'rgba(255,255,255,.7)'}}}},scales:{{x:{{grid:{{color:'rgba(255,255,255,.05)'}},ticks:{{color:'rgba(255,255,255,.45)',font:{{size:10}}}}}},y:{{grid:{{color:'rgba(255,255,255,.05)'}},ticks:{{color:'rgba(255,255,255,.45)',font:{{size:10}}}},beginAtZero:true}}}}}};}}
function ds(label,data,color){{return{{label,data,borderColor:color,backgroundColor:color+'22',borderWidth:2.5,pointBackgroundColor:color,pointRadius:4,pointHoverRadius:6,tension:.35,fill:true}};}}
function mkLine(id,labels,datasets,legend=false){{
  var o=opts(); if(legend)o.plugins.legend={{display:true,labels:{{color:'rgba(255,255,255,.6)',font:{{size:11}}}}}};
  new Chart(document.getElementById(id),{{type:'line',data:{{labels,datasets}},options:o}});
}}
mkLine('chartMonthly',MONTHS,[ds('Admissions',MONTHLY,'#14b8a6')]);
mkLine('chartStatus',MONTHS,[ds('Examined',EX_M,'#22c55e'),ds('Pending',PEND_M,'#fbbf24')],true);
...
.topbar{padding:1.1rem 1.75rem;border-bottom:1px solid rgba(255,255,255,.07);
  display:flex;align-items:center;justify-content:space-between;
  background:rgba(10,22,40,.85);backdrop-filter:blur(10px);position:sticky;top:0;z-index:50}
...
.stat.t .stat-val{color:var(--teal-light)}.stat.g .stat-val{color:var(--gold)}.stat.gr .stat-val{color:#86efac}
.tw{background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.08);border-radius:18px;overflow:hidden}
.th{padding:1.1rem 1.25rem;border-bottom:1px solid rgba(255,255,255,.07);display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:.5rem}
table{width:100%;border-collapse:collapse}
thead th{text-align:left;padding:.75rem 1rem;font-size:.7rem;color:rgba(255,255,255,.3);text-transform:uppercase;letter-spacing:.1em;border-bottom:1px solid rgba(255,255,255,.06);font-weight:400}
...
.srch{background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.1);border-radius:10px;padding:.55rem .9rem;color:#fff;font-size:.83rem;width:200px;outline:none;font-family:inherit}
.srch:focus{border-color:var(--teal)}.srch::placeholder{color:rgba(255,255,255,.25)}
.fb{background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.1);color:rgba(255,255,255,.45);padding:.5rem .9rem;border-radius:9px;font-size:.78rem;cursor:pointer;font-family:inherit;transition:all .2s}
...
<div class="main">
  ...
  <div class="content">
    ...
    <div style="background:rgba(13,148,136,.07);border:1px solid rgba(13,148,136,.2);border-radius:12px;padding:.8rem 1.1rem;margin-bottom:1.25rem;font-size:.82rem;color:rgba(255,255,255,.5);display:flex;align-items:center;gap:.5rem">
      <span style="color:var(--teal-light);font-size:1rem">💡</span>
      Click any patient row to open the <strong style="color:var(--teal-light)">Examination Page</strong>.
    </div>
    ...
    <div class="tw">
      ...
      <div style="overflow-x:auto">
        <table>
          <thead><tr><th>#</th><th>Patient Name</th><th>Date</th><th>Time</th><th>Symptoms</th><th>Status</th><th>Actions</th></tr></thead>
...
def doctor_register():
    if request.method == "POST":
        ...
        if not name or not pw:
            return page_doctor_register(error="Name and password are required")
        if len(pw) < 6:
            return page_doctor_register(error="Password must be at least 6 characters")
        ...
        try:
            with get_db() as db:
                ...
                db.commit()
            return page_doctor_register(success_qr=qr_id, success_name=name)
        except sqlite3.IntegrityError:
            return page_doctor_register(error="Doctor name already exists")
    return page_doctor_register()
...
def doctor_login():
    if request.method == "POST":
        ...
        return page_doctor_login(error="Invalid name or password")
    return page_doctor_login()

# ── QR Verify (AJAX) ─────────────────────────
@app.route("/doctor/qr-verify", methods=["POST"])
def doctor_qr_verify():
    ...
    if doc:
        session["doctor_id"]   = doc["id"]
        session["doctor_name"] = doc["name"]
        return jsonify({"success": True, "name": doc["name"]})
...
def doctor_dashboard():
    ...
    return page_dashboard(session["doctor_name"], [dict(r) for r in pts])
...
def doctor_statistics():
    ...
    return page_statistics(session["doctor_name"], [dict(r) for r in pts])

# ── Examine Patient ──────────────────────────
@app.route("/doctor/patient/<int:pid>/examine", methods=["GET", "POST"])
@login_required
def doctor_examine(pid):
    with get_db() as db:
        pt = db.execute("SELECT * FROM patients WHERE id=?", (pid,)).fetchone()
    if not pt:
        return "<h1 style='font-family:sans-serif;padding:2rem'>Patient not found</h1>", 404
    ...
    if request.method == "POST":
        ...
        if not diagnosis or not prescription:
            return page_examine(session["doctor_name"], dict(pt), success=False)
        ...
        with get_db() as db:
            ...
            db.commit()
            pt = db.execute("SELECT * FROM patients WHERE id=?", (pid,)).fetchone()
        return page_examine(session["doctor_name"], dict(pt), success=True)

    return page_examine(session["doctor_name"], dict(pt))
...
def patient_admission():
    if request.method == "POST":
        ...
        if request.is_json:
            ...
            with get_db() as db:
                db.execute(
                    "INSERT INTO patients (name,date_of_appointment,appointment_time,symptoms) VALUES (?,?,?,?)",
            ...
            if not all([name, date, time_val, symptoms]):
                return page_patient_admission(error="All fields are required")
            with get_db() as db:
                db.execute(
                    "INSERT INTO patients (name,date_of_appointment,appointment_time,symptoms) VALUES (?,?,?,?)",
                    (name, date, time_val, symptoms)
                ...
                db.commit()
            return page_patient_success(name, date, time_val, symptoms)
    return page_patient_admission()
...
if __name__ == "__main__":
    ...
    print()
    print("=" * 50)
    print("   MediCare Clinic System (Flask)")
    print("=" * 50)
